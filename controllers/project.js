...
var controller = {
  ...
  getProjects: function(req, res){
    ...
    Project.find({}).sort('+year').exec((err, projects) => {
      ...
      if(err) return res.status(500).send({message: 'Error al listar los proyectos'});
  ...
  updateProject: function(req, res){
    ...
    Project.findByIdAndUpdate(projectId, update, {new:true}, (err, projectUpdated) => {
      ...
      if(!projectUpdated) res.status(404).send({message: 'No se ha podido actualizar el proyecto'});
  ...
  deleteProject: function(req, res){
    ...
    Project.findByIdAndRemove(projectId, (err, projectRemoved) => {
      ...
      if(!projectRemoved) res.status(404).send({message: 'No se puede borrar el proyecto'});
